...
for i in range(28):
    mps = MPS(input_dim=28, output_dim=10, bond_dim=bond_dim, 
          adaptive_mode=adaptive_mode, periodic_bc=periodic_bc, path=list(range(i*28,(i+1)*28)))
    mps_list.append(mps)


    mps = MPS(input_dim=28, output_dim=10, bond_dim=bond_dim, 
          adaptive_mode=adaptive_mode, periodic_bc=periodic_bc, path=list(range(i,28**2,28)))
    mps_list.append(mps)
